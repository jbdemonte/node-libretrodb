...
export class Libretrodb<T extends Buffer | string> {
  private metadataOffset: number = 0;
  private firstIndexOffset: number = 0;
  private count: number = 0;
  private file: FileHandler;
  private entries: Entry<T>[] = [];
  private index: { [hash: string]: Entry<T> } = {};
  private options: Options;


  static async from<Ops extends Partial<Options>>(path: string, options?: Ops) {
    const db = new Libretrodb<Ops extends { bufferToString: false } ? Buffer : string>(path, options);
    await db.load();
    return db;
  }

  constructor(private path: string, options: Partial<Options> = {}) {
    ...
    return this.entries.find(entry => {
      ...
      if (value) {
        if (this.options.bufferToString) {
          ...
          return (value as Buffer).compare(search) === 0
        }
      }
    });
  }
  ...
  private addToIndex(entry: Entry<T>) {
    const keys: Array<keyof Entry<T>> = ['crc', 'md5', 'sha1'];
